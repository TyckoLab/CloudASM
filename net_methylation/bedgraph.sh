...
bq query \
    ...
    --replace=true \
    "WITH OB AS (
            SELECT chr, pos-1 as pos, IF(meth_call = 'Z',1,0) as meth, 1 as cov
            FROM ${DATASET_ID}.${SAMPLE}_CpGOB),
          OT AS (
            SELECT chr, pos, IF(meth_call = 'Z',1,0) as meth, 1 as cov
            FROM ${DATASET_ID}.${SAMPLE}_CpGOT)
          SELECT * FROM OB
          UNION ALL 
          SELECT * FROM OT"
...
bq query \
    ...
    --replace=true \
    " SELECT chr, pos as pos_start, pos as pos_end, 
             SUM(meth)/SUM(cov) as meth_perc, SUM(cov) as cov
        FROM ${DATASET_ID}.${SAMPLE}_tmp
        GROUP BY chr, pos"
...
bq query \
    ...
    --replace=true \
    " SELECT chr, pos_start, pos_end, meth_perc
        FROM ${DATASET_ID}.${SAMPLE}_both_context
        WHERE cov >= 10"
...
bq query \
    ...
    --replace=true \
    " SELECT chr, pos_start, pos_end, cov
        FROM ${DATASET_ID}.${SAMPLE}_both_context
        WHERE cov >= 10"
